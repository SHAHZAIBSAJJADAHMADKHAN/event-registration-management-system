...
const rows = [{ registration_id: "reg-a", attendee_name: "Alice Attendee", attendee_email: "alice@example.test", registration_status: "active", registered_at: "2030-01-01T09:00:00Z" }, { registration_id: "reg-b", attendee_name: "Bob Cancelled", attendee_email: "bob@example.test", registration_status: "cancelled", registered_at: "2030-01-01T09:30:00Z" }];
...
describe("AdminAttendeesPage", () => {
  ...
  it("supports search and status filtering", async () => { renderPage(); await screen.findByText("Alice Attendee"); fireEvent.change(screen.getByLabelText("Search attendees"), { target: { value: "bob" } }); expect(screen.queryByText("Alice Attendee")).not.toBeInTheDocument(); await screen.findByText("Bob Cancelled"); fireEvent.change(screen.getByLabelText("Registration status"), { target: { value: "active" } }); await screen.findByText("No attendees match this search or filter."); });
